...
contract Microsponsors {
  ...
  */
  ...
  // at any time.
  mapping (uint256 => address) public sponsorSlotToApproved;
  ...
  */
  ...
  function tokensOfOwner(address _owner) external view returns (uint256[] tokenIds) {
    ...
    if (tokenCount == 0) {
      ...
      uint256 totalSlots = totalSupply() - 1;
      uint256 resultIndex = 0;

      for (uint256 counter = 0; counter <= totalSlots; counter++) {
          // if (sponsorSlots[counter].exists) {
            if (sponsorSlots[counter].owner == _owner) {
              result[resultIndex] = sponsorSlots[counter].id;
              resultIndex++;
            }
          // }
      }
  ...
  function _approve(uint256 _tokenId, address _approved) internal {
    sponsorSlotToApproved[_tokenId] = _approved;
  }

  // TODO: for demo purposes, we may want to disable this check
  function _approvedFor(address _claimant, uint256 _tokenId) internal view returns (bool) {
    return sponsorSlotToApproved[_tokenId] == _claimant;
  }
  ...
  function _transfer(address _from, address _to, uint256 _tokenId) internal {
      ...
      if (_from != address(0)) {
        ...
        delete sponsorSlotToApproved[_tokenId];
